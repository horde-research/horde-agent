...
Copied from `agentic_train_pipeline/training/tuning.py` and adjusted for new package layout.
"""

from dataclasses import dataclass

from core.agentic.schemas import TrainingAdjustmentDecision
...
def generate_random_candidates(base: TrainConfig, bounds: TuningBounds, num_trials: int, seed: int) -> list[TrainConfig]:
    import random
...
def apply_adjustments(config: TrainConfig, decision: TrainingAdjustmentDecision, bounds: TuningBounds) -> TrainConfig:
    lr = min(max(config.lr * decision.lr_multiplier, bounds.min_lr), bounds.max_lr)
    batch_size = min(
        max(config.batch_size + decision.batch_size_delta, bounds.min_batch_size),
        bounds.max_batch_size,
    )
    grad_accum = min(
        max(config.grad_accum + decision.grad_accum_delta, bounds.min_grad_accum),
        bounds.max_grad_accum,
    )
